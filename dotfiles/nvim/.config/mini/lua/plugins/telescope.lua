return {
  ...
  opts = {
    defaults = {
      ...
      -- file_ignore_patterns applies to find_files
  ...
  config = function(_, opts)
    ...
    local function load_ext(name)
      ...
      if not ok then
        vim.notify("telescope: failed to load extension '" .. name .. "'\n" .. err, vim.log.levels.WARN)
      end
    end
    ...
    -- Smart git-aware file search: checks git status explicitly rather than catching
    ...
    -- Frecency picker: reflects actual cwd at call time (fixes startup-frozen project path)
