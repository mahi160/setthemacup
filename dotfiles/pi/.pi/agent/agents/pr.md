...
4. Run the `thermo-nuclear-code-quality-review` skill against the diff before drafting anything, if available. Presumptive blockers found (file-size explosion, spaghetti branching, code-judo opportunity, etc.) — surface them to the user and ask whether to fix first or proceed as-is. Don't silently skip this, don't silently block on it either.
