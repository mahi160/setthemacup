...
#   - raycast-backup: daily 2am         — backup Raycast 2 settings to git
#
# Usage:
...
RAYCAST_JOB="0 2 * * * SETTHEMACUP=${REPO} ${REPO}/scripts/raycast-backup.sh >> /tmp/raycast-backup.log 2>&1"
...
echo "$RAYCAST_JOB" >> "$TMPFILE"
crontab "$TMPFILE"

echo "Cron entries installed:"
crontab -l | grep -E "pi-prune|fnm_multishells|fnm-clean|raycast-backup"